...
const parsePagination = (query) => {
  ...
  const limit = Number.isNaN(parsedLimit) || parsedLimit < 1 ? DEFAULT_LIMIT : Math.min(parsedLimit, MAX_LIMIT);
...
const getCurrentUserId = (req) => {
  ...
  if (!userId) {
    throw createError("User ID missing in headers", 401);
  }
...
export const getSubscriptions = async (req, res, next) => {
  try {
    ...
    console.log("[subscriptions:list]", { userId, count: subscriptions.length });
...
export const getUserSubscriptions = async (req, res, next) => {
  try {
    ...
    return getSubscriptions(req, res, next);
  } catch (error) {
    next(error);
  }
};
...
export const getSubscription = async (req, res, next) => {
  try {
    const userId = getCurrentUserId(req);
    const subscription = await getOwnedSubscription(req.params.id, userId);
...
export const createSubscription = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.error("[subscriptions:create] workflow trigger failed:", workflowError.message);
    }
...
export const getUpcomingRenewals = async (req, res, next) => {
  try {
    ...
    const [subscriptions, total] = await Promise.all([
      Subscription.find(filter).sort({ renewalDate: 1 }).skip(skip).limit(limit),
      Subscription.countDocuments(filter),
    ]);

    console.log("[subscriptions:upcoming]", { userId, count: subscriptions.length });
...
// // 🔥 IMPORTANT: Updated to use Clerk userId
// const getOwnedSubscription = async (subscriptionId, userId) => {
//   const subscription = await Subscription.findOne({
//     _id: subscriptionId,
//     userId: userId,
...
//       ...sub.toObject(),
//       id: sub._id.toString(), // 👈 VERY IMPORTANT FOR FRONTEND
//     }));
